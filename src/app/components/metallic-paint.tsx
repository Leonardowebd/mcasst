/**
 * MetallicPaint — TypeScript port of the React Bits component
 * Inspired by Paper's Liquid Metal effect.
 * No external CSS file — styles are injected once.
 */
import { useEffect, useRef, useState, useCallback } from "react";

/* ── inject CSS once ─────────────────────────────────────────────────── */
if (typeof document !== "undefined") {
  const id = "metallic-paint-styles";
  if (!document.getElementById(id)) {
    const s = document.createElement("style");
    s.id = id;
    s.textContent = `.paint-container{display:block;height:100%;width:100%;object-fit:contain;}`;
...
void main(){
  ...
  vec2 iC=fA();
  vec4 ts=texture(u_tex,iC);
  float dp=ts.r,shapeMask=ts.a;
  ...
  fl*=.12+(1.05-lD)*cA;fl*=smoothstep(1.,.65,lD);
  ...
  rO+=vM1*cM1*4.5;rO-=sl;
  float bO=rB*1.25;
  float vM2=smoothstep(-.02,.35,sc.y)*smoothstep(.75,.08,sc.y);
  float cM2=smoothstep(.35,.55,cv)*smoothstep(.75,.35,cv);
  bO+=vM2*cM2*.9;bO-=lD*.18;
  rO*=u_refract*u_chroma;bO*=u_refract*u_chroma;
  float sf=u_blur;
  float rP=fract(fl+rO),rC=mG(hi.r,lo.r,rP,sf+.018+u_refract*cv*.025,cv);
  float gP=fract(fl),gC=mG(hi.g,lo.g,gP,sf+.008/max(.01,1.-sl),cv);
  float bP=fract(fl-bO),bC=mG(hi.b,lo.b,bP,sf+.008,cv);
  vec3 col=vec3(rC,gC,bC);
  col=(col-.5)*u_contrast+.5;col=clamp(col,0.,1.);
...
function processImage(img: HTMLImageElement): ImageData {
  ...
  let w = img.naturalWidth || (img as any).width || 500;
  let h = img.naturalHeight || (img as any).height || 500;

  if (w > MAX || h > MAX || w < MIN || h < MIN) {
    const scale = w > h
      ? w > MAX ? MAX / w : w < MIN ? MIN / w : 1
      : h > MAX ? MAX / h : h < MIN ? MIN / h : 1;
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      if (x===0||x===w-1||y===0||y===h-1||!shapeMask[idx-1]||!shapeMask[idx+1]||!shapeMask[idx-w]||!shapeMask[idx+w])
        boundaryMask[idx] = 1;
    }
  }

  const u = new Float32Array(size);
  const ITER = 200, C = 0.01, omega = 1.85;
    ...
    for (let y = 1; y < h-1; y++) {
      for (let x = 1; x < w-1; x++) {
        ...
        const sum = (shapeMask[idx+1]?u[idx+1]:0)+(shapeMask[idx-1]?u[idx-1]:0)
                  + (shapeMask[idx+w]?u[idx+w]:0)+(shapeMask[idx-w]?u[idx-w]:0);
  ...
  for (let i = 0; i < size; i++) {
    ...
    out.data[px+3]=Math.round(alphaValues[i]*255);
  }
  return out;
}
...
export default function MetallicPaint({
  imageSrc,
  seed = 42, scale = 4, refraction = 0.01, blur = 0.015,
  liquid = 0.75, speed = 0.3, brightness = 2, contrast = 0.5,
  angle = 0, fresnel = 1,
  lightColor = "#ffffff", darkColor = "#000000",
  patternSharpness = 1, waveAmplitude = 1, noiseScale = 0.5,
  chromaticSpread = 2, mouseAnimation = false,
  distortion = 1, contour = 0.2, tintColor = "#c0c0c0",
}: MetallicPaintProps) {
  ...
  useEffect(() => {
    ...
    gl.uniform3f(u.u_lightColor, lr,lg,lb);
    gl.uniform3f(u.u_darkColor,  dr,dg,db);
  ...
  useEffect(() => {
    ...
    const gl = glRef.current!, u = uniformsRef.current;
    const canvas = canvasRef.current!, mouse = mouseRef.current;

    const handleMouse = (e: MouseEvent) => {
      const r = canvas.getBoundingClientRect();
      mouse.targetX = (e.clientX - r.left) / r.width;
      mouse.targetY = (e.clientY - r.top)  / r.height;
    };
